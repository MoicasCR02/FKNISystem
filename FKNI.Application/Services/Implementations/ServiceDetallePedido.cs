...
namespace FKNI.Application.Services.Implementations
{
    public class ServiceDetallePedido : IServiceDetallePedido
    {
        ...
        public async Task<ICollection<DetallePedidoDTO>> FindByIdAsync(int id_pedido)
        {
            var @object = await _repository.FindByIdAsync(id_pedido);
            var objectMapped = _mapper.Map<ICollection<DetallePedidoDTO>>(@object);
            return objectMapped;
        }
        ...
        public async Task<int> AddAsync(DetallePedidoDTO dto)
        {
            // Map LibroDTO to Libro
            var objectMapped = _mapper.Map<DetallePedido>(dto);
            // Return
